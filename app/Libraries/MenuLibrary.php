...
class MenuLibrary  {
    ...
    public function createMenu(){
        ...
        $contexts = \App\Modules\developer\context\Models\ContextModel::where('is_nav_bar', 1)
        ->where('flag', 1)
        ->orderBy('order')->get();
...
//        ->where('flag', 1)
    ...
    private function createSubMenu($context, $context_id){
        ...
        $modules = \App\Modules\developer\context\Models\ContextModel::find($context_id)->modules()
                ->where('id_parent', '0')
                ->where('flag', 1)
                ->orderBy('order')->get();
        foreach($modules as &$module ){
            ...
            if (\PermissionsLibrary::hasPermission('mod-'.strtolower($module_name).'-index')){
                ...
                $str_submenu.="<li><a id='menu-akhir' href='".url().$module->path."'><i class='fa ".$icon."'></i>".(($module->name!='Tahun Pendaftaran')?$module->name:'Pengaturan Pendaftaran')."</a></li>";
            }
        }
        $str_submenu.="</ul>";
        return $str_submenu;
