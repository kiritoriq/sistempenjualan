...
class PDF_MC_Table extends FPDFS
{
    ...
    public $default_cell_w = 200;
    public $default_cell_h = 7;
    ...
    public function PrintCell($txt='', $border=0, $ln=1, $align='', $fill=false, $link='')
    {
        return $this->Cell(
            $this->default_cell_w,
            $this->default_cell_h,
    ...
    public function Row($data, $lebar = 8)
    {
        $nb=0;
    ...
    public function RowNoLinesHeader($data)
    {
        $nb=0;
        for ($i=0;$i<count($data);$i++) {
            $nb=max($nb, $this->NbLines($this->widths[$i], $data[$i]));
    ...
    public function RowNoLines($data, $lebar=8)
    {
        $nb=0;
        for ($i=0;$i<count($data);$i++) {
            $nb=max($nb, $this->NbLines($this->widths[$i], $data[$i]));
        ...
        for ($i=0;$i<count($data);$i++) {
            $w=$this->widths[$i];
            $a=isset($this->aligns[$i]) ? $this->aligns[$i] : 'L';
            $b=isset($this->border[$i]) ? $this->border[$i] : '';
            //Save the current position
            $x=$this->GetX();
            $y=$this->GetY();
            //Draw the border
//            $this->Rect($x,$y,$w,$h);
            //Print the text
            $this->MultiCell($w, $lebar, $data[$i], $b, $a);
            //Put the position to the right of the cell
            $this->SetXY($x+$w, $y);
        }
        //Go to the next line
